...
void set_track_id(detection *new_dets, int new_dets_num, float thresh, float sim_thresh, float track_fiou_norm, int deque_size, int dets_for_track, int dets_for_show)
{
    ...
    for (int index = 0; index < new_dets_num*old_dets.size(); ++index) {
        ...
        //printf(" fiou = %f \n", box_fiou(new_dets[new_id].bbox, old_dets[old_id].bbox));
        if (track_idx[track_id] && new_idx[new_id] && old_idx[old_id] && check_classes_id(new_dets[new_id], old_dets[old_id], thresh)) {
            ...
            //float fiou = box_fiou(new_dets[new_id].bbox, old_dets[old_id].bbox);
            float fiou = box_iou(new_dets[new_id].bbox, old_dets[old_id].bbox);
            sim = sim * (1 - track_fiou_norm) + fiou * track_fiou_norm;
