...
float box_fiou(box a, box b)
{
  ...
  //float ar_loss = 4 / (M_PI * M_PI) * (atan(ar_gt) - atan(ar_pred)) * (atan(ar_gt) - atan(ar_pred));
    float ar_loss = 8 / (M_PI * M_PI) * ((atan(b.w/a.w) - (M_PI/4)) * (atan(b.w/a.w) - (M_PI/4)) + (atan(b.h/a.h) - (M_PI/4)) * (atan(b.h/a.h) - (M_PI/4)));
    float alpha = ar_loss / (1 - iou + ar_loss + 0.000001);
    float fiou_term = d + alpha * ar_loss;                   //fiou
#ifdef DEBUG_PRINTS
    printf("  c: %f, u: %f, riou_term: %f\n", c, u, fiou_term);
#endif
    return iou - fiou_term;
}
...
//#endif
    ...
    //The following codes are calculating the gradient of fiou.

    if (iou_loss == FIOU) {
    float ar_gt = truth.w / truth.h;
        float ar_pred = pred.w / pred.h;
        float ar_loss = 8 / (M_PI * M_PI) * ((atan(truth.w/pred.w) - (M_PI/4)) * (atan(truth.w/pred.w) - (M_PI/4)) + (atan(truth.h/pred.h) - (M_PI/4)) * (atan(truth.h/pred.h) - (M_PI/4)));
    float alpha = ar_loss / (1 - I/U + ar_loss + 0.000001);
    float ar_dw=8/(M_PI*M_PI)*((atan(truth.w/pred.w) - (M_PI/4)) * (atan(truth.w/pred.w) - (M_PI/4)) + (atan(truth.h/pred.h) - (M_PI/4)) * (atan(truth.h/pred.h) - (M_PI/4)))*pred.h;
        float ar_dh=-8/(M_PI*M_PI)*((atan(truth.w/pred.w) - (M_PI/4)) * (atan(truth.w/pred.w) - (M_PI/4)) + (atan(truth.h/pred.h) - (M_PI/4)) * (atan(truth.h/pred.h) - (M_PI/4)))*pred.w;
        if (C > 0) {
        // dar*
            p_dx += (2*(truth.x-pred.x)*C-(2*Cw*dCw_dx+2*Ch*dCh_dx)*S) / (C * C);
            p_dy += (2*(truth.y-pred.y)*C-(2*Cw*dCw_dy+2*Ch*dCh_dy)*S) / (C * C);
